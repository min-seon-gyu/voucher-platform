...
- **결산 주기 일괄 정산 배치(Spring Batch)**: 기준일(`referenceDate`) 파라미터로 **APPROVED 가맹점 전체**를 청크 처리해 각 가맹점 지역의 정산주기(일/주/월) 구간 정산을 일괄 생성한다.
  - **재실행 멱등**: 이미 존재하는 구간은 스킵(unique 제약 + 프로세서 사전 체크 이중 방어), 0원 정산 스킵
  - **내결함성**: 단건 가맹점 실패는 `skip`으로 격리하고 결산 전체는 완주(운영 모니터링 대상)
  - **검증 스텝**: 결산 후 PENDING 정산의 이상치(0 이하 금액)를 자동 점검 + 메트릭(`settlement.batch.*`) — 마감 검증 자동화의 최소 형태
  - **트리거**: 매일 03:00(KST) 스케줄러(전일 기준) + 관리자 수동 실행 `POST /api/v1/admin/settlements/batch?referenceDate=`
  - **메타 스키마**: Flyway V8이 소유(`spring.batch.jdbc.initialize-schema=never`), 起動 자동 실행 차단(`spring.batch.job.enabled=false`)
...
| **정산배치(admin)** | POST | `/api/v1/admin/settlements/batch` | 결산 주기 일괄 정산 배치 실행 (ADMIN, Spring Batch) |
...
| Batch | Spring Batch 5.1 (결산 주기 일괄 정산 Job — 청크·skip·재실행 멱등·검증 스텝) |
| DB | MySQL 8 + Flyway (V1→V8, `ddl-auto: validate`) |
