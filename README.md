...
**원장 기록(결제·쿠폰 보조분·포인트 적립)은 Kafka로 전환하지 않는다.** 잔액 변경과 원장 기록은 같은 DB 트랜잭션에서 동기 처리해야 정합성이 보장된다. 이벤트 기반으로 전환하면 커밋 후 리스너 실행 전 장애 시 원장 누락이 발생한다.
...
| **상품권** | GET | `/api/v1/vouchers` | 목록 조회 (QueryDSL + 페이지네이션) |
| | GET | `/api/v1/vouchers/{id}` | 단건 조회 |
...
| Infra | Docker Compose (MySQL + Redis + Prometheus + Grafana), Dockerfile, GitHub Actions CI |
...
- **외부화된 환경변수**: `DB_HOST/DB_PORT/DB_NAME/DB_USERNAME/DB_PASSWORD`, `REDIS_HOST/REDIS_PORT`, `JWT_SECRET`.
